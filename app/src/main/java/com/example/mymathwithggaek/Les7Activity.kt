...
class Les7Activity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        enableEdgeToEdge()
        setContentView(R.layout.activity_les7)
        ...
        val backl7 : ImageView = findViewById(R.id.backl7)
...
//для проверки на то проходили этот уровень или нет(обращаемся по ключу и проверяем) по нажатию кнопки
        finish.setOnClickListener {
            val Fix = sharedPreferences.getString("TroobleL7", "")
            if (Fix != "0") {
                if (
                    (Ans1.text.toString().trim() == "2") &&
                    (Ans2.text.toString().trim() == "1")  &&
                    (Ans3.text.toString().trim() == "3") &&
                    (Ans4.text.toString().trim() == "7") &&
                    (Ans5.text.toString().trim() == "3")  &&
                    (Ans6.text.toString().trim() == "-1")
                    ) {
